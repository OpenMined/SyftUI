...
export function UploadProgress({ uploads, onClear }: UploadProgressProps) {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-4 right-4 w-80 bg-card border border-border rounded-lg shadow-lg overflow-hidden z-50"
    >
      <div className="p-3 border-b border-border flex items-center justify-between">
        ...
        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => uploads.forEach((u) => onClear(u.id))}>
          <X className="h-4 w-4" />
        </Button>
      </div>
          ...
          {uploads.map((upload) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <div className="text-sm truncate flex-1 mr-2">{upload.name}</div>
                <div
                  className={cn(
                    "text-xs font-medium",
