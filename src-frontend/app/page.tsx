...
export default function HomePage() {
  ...
  const [step, setStep] = useState<
    "connect" | "configure" | "email" | "verify"
  >("connect");
  ...
  const { updateSettings, status, connect, displayUrl } = useConnectionStore();
      ...
      <Card className="mx-auto max-w-md">
        ...
        <CardContent>
          {step === "connect" && (
            <Form {...connectionForm}>
              <form
                onSubmit={connectionForm.handleSubmit(handleConnectClient)}
                className="space-y-4"
              >
                <FormField
          ...
          {step === "configure" && (
            <Form {...configForm}>
              <form
                onSubmit={configForm.handleSubmit(handleConfigureClient)}
                className="space-y-4"
              >
                <FormField
          ...
          {step === "email" && (
            <Form {...configForm}>
              <form
                onSubmit={configForm.handleSubmit(handleSendVerification)}
                className="space-y-4"
              >
                <FormField
                  control={configForm.control}
          ...
          {step === "verify" && (
            <Form {...verificationForm}>
              <form
                onSubmit={verificationForm.handleSubmit(handleCompleteSetup)}
                className="space-y-4"
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        Please enter the token sent to{" "}
                        {configForm.getValues("email")}
                      </FormLabel>
                      <FormControl>
                        <Input
                      ...
                      <p className="text-muted-foreground text-sm">
                        Also check your spam folder if you don&apos;t see the
                        email.
                      </p>
                    </FormItem>
                  )}
